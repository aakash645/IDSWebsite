...
import { Breadcrumb, SolutionSidebar, SolutionHero, softwarePlatformsMenu } from '@/components/solutions';
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <main className="w-full lg:w-3/4 order-1 lg:order-2">
            <FadeIn>
              <SolutionHero title={pageTitle} image="https://images.unsplash.com/photo-1547658719-da2b51169166?q=80&w=2070&auto=format&fit=crop" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                {capabilities.map((cap, i) => (
                  <div key={i} className="flex items-start gap-4 p-4 rounded-xl bg-white/5 border border-white/5 hover:border-white/20 transition-colors">
                    <div className="mt-1 w-6 h-6 rounded-full bg-white/10 flex items-center justify-center text-ids-blue shrink-0"><CircleCheck size={14} /></div>
                    <div><h4 className="font-bold text-white mb-1">{cap.title}</h4><p className="text-sm text-gray-400">{cap.desc}</p></div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-gradient-to-r from-white/5 to-transparent border border-white/10 rounded-3xl p-8 md:p-12 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex flex-wrap gap-4">
                    <Link href="/contact" className="bg-white text-black font-bold px-8 py-3 rounded-full hover:bg-gray-200 transition-colors flex items-center gap-2">Request Demo <ArrowRight size={18} /></Link>
                    <button className="bg-transparent border border-white/20 text-white font-bold px-8 py-3 rounded-full hover:bg-white/10 transition-colors">Download Datasheet</button>
                  </div>
                </div>
              </div>
            </FadeIn>
